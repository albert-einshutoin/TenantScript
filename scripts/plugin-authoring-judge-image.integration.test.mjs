...
const handlerBodies = {
  "approval-invoice-threshold": `const value = record(payload)?.totalCents; if (typeof value !== "number" || !Number.isSafeInteger(value) || value < 0) throw new Error("invalid_invoice_total"); return value >= 100000 ? { decision: "deny", reasonCode: "approval_required" } : { decision: "allow", reasonCode: "allowed" };`,
  "approval-refund-review": `const value = record(payload)?.purchaseAgeDays; return typeof value === "number" && Number.isSafeInteger(value) && value >= 0 && value <= 30 ? { decision: "allow", reasonCode: "allowed" } : { decision: "deny", reasonCode: "approval_required" };`,
  "capability-github-issue": `const value = record(payload); const id = bounded(value?.deploymentId, 64); const reason = bounded(value?.reason, 160); if (id === undefined || reason === undefined) throw new Error("invalid_deployment"); try { await context.capability("github.issue.create", { title: \`Deployment \${id} failed\`, body: reason }); } catch { throw new Error("issue_creation_failed"); } return { status: "accepted" };`,
  ...
  "error-malformed-payload": `const value = record(payload); const customerId = bounded(value?.customerId, 64); const displayName = bounded(value?.displayName, 100); if (customerId === undefined || displayName === undefined) throw new Error("invalid_customer"); return { status: "transformed", output: { customerId, displayName } };`,
  ...
  "policy-api-method-allowlist": `const value = record(payload); const keys = value === undefined ? [] : Object.keys(value); const allowed = keys.length === 2 && keys.includes("method") && keys.includes("url") && (value?.method === "GET" || value?.method === "POST") && typeof value?.url === "string" && (value.url === "https://api.example.com" || value.url.startsWith("https://api.example.com/")); return allowed ? { decision: "allow", reasonCode: "allowed" } : { decision: "deny", reasonCode: "request_not_allowed" };`,
  "policy-data-residency": `return record(payload)?.region === "eu-west" ? { decision: "allow", reasonCode: "allowed" } : { decision: "deny", reasonCode: "region_not_allowed" };`,
  "webhook-currency-normalizer": `const value = record(payload); const currency = value?.currency; const amount = value?.amount; if (typeof currency !== "string" || !/^[A-Za-z]{3}$/.test(currency) || typeof amount !== "number" || !Number.isSafeInteger(amount) || amount < 0) throw new Error("invalid_payment"); return { status: "transformed", output: { currency: currency.toUpperCase(), amount } };`,
  "webhook-ticket-priority": `const value = record(payload)?.priority; const priorities: Record<string, number> = { low: 1, normal: 2, high: 3, urgent: 4 }; return { status: "transformed", output: { priority: typeof value === "string" ? priorities[value] ?? 2 : 2 } };`
};
