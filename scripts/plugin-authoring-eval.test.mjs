...
test("wires eval drift, security checks, documentation, and Tier 1", () => {
  ...
  assert.equal(
    ...
    "pnpm --filter @tenantscript/cli... build && node --test scripts/plugin-authoring-eval.test.mjs scripts/plugin-authoring-eval-security.test.mjs scripts/plugin-authoring-isolated-runner.test.mjs scripts/plugin-authoring-isolated-runner-security.test.mjs"
  );
