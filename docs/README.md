...
- **Repository verified / Blocked** — [Plugin authoring eval contract](reference/plugin-authoring-evals.md)で、10件の固定corpus、決定論的judge、pass@1と失敗分類を検証できます。[Isolated judge runner](reference/plugin-authoring-isolated-runner.md)は固定baseline・network deny・resource limit・cleanup契約までrepository検証済みですが、review済みjudge imageの公開と外部agent計測は未完です。
...
- **Repository verified** — [Manifest v1 Portable Specification](spec/manifest-v1.md)で、runtime非依存のauthority宣言、semantic rule、JSON conformance protocolと非保証を確認できます。
...
- **Repository verified / Blocked** — [Template gallery](community/template-gallery.md)で、静的catalogの検索・互換情報・accessibility E2E、Workers Assets dry-run、保護されたmanual release契約を確認できます。実live deployと再利用数は未検証です。
