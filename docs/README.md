...
- **Repository verified** — [Slack OAuth callback composition](operations/slack-oauth-callback.md)で、state-first sequencing、server-owned scope、secret-freeな接続合成を確認できます。
- **Repository verified** — [Slack OAuth install-start](operations/slack-oauth-install-start.md)で、認証済みtenant scope、固定Slack認可URL、`Secure` / `HttpOnly` / `SameSite=Lax` Cookieを確認できます。
- **Repository verified / Blocked** — [Slack OAuth v2 exchange boundary](operations/slack-oauth-exchange.md)で、固定originのcode交換contractと未実装のcallback/live composition境界を区別できます。
