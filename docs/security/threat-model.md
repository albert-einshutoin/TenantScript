...
| raw egress, runaway execution, memory exhaustion, or subrequest exhaustion | deny-by-default fetch plus worker memory, timeout, and call budgets                                                                   | security boundary cases in [`packages/loader/test/security-suite.test.ts`](../../packages/loader/test/security-suite.test.ts), CPU/memory/recursion and broker failures in [`packages/loader/test/chaos-suite.test.ts`](../../packages/loader/test/chaos-suite.test.ts), and hook-specific failure behavior in [`packages/host-sdk/test/chaos-suite.test.ts`](../../packages/host-sdk/test/chaos-suite.test.ts) |
| repeated failure or timeout continuing after a runaway threshold           | Per-installation D1 counters atomically disable once and require explicit recovery                                                    | policy cases in [`packages/control-plane/test/runaway-guard.test.ts`](../../packages/control-plane/test/runaway-guard.test.ts) and concurrent D1 transitions in [`packages/control-plane/test/runaway-guard.workers.test.ts`](../../packages/control-plane/test/runaway-guard.workers.test.ts)                                                                                                                  |
...
| provider token disclosure at rest or encrypted-record movement             | Per-record DEKs encrypt tokens; deployment KEKs wrap DEKs; authenticated data binds both layers to the complete secret ref            | plaintext, ref-movement, tampering, malformed-envelope, and key-rotation cases in [`packages/control-plane/test/secret-store.test.ts`](../../packages/control-plane/test/secret-store.test.ts)                                                                                                                                                                                                                  |
...
| App database route fallback or binding reuse                               | exact app-to-binding router rejects unknown apps, duplicates, and shared handles; Admin routes share an exhaustive isolation registry | [`app-database-router.test.ts`](../../packages/control-plane/test/app-database-router.test.ts), [`tenant-isolation-matrix.test.ts`](../../packages/control-plane/test/tenant-isolation-matrix.test.ts), and [ADR-004](../adr/004-app-database-sharding.md)                                                                                                                                                      |
| D1 or R2 outage producing unsafe fallback state                            | state-dependent operations and incomplete archive searches fail closed; hot D1 data remains authoritative                             | outage injection cases in [`packages/control-plane/test/chaos-suite.test.ts`](../../packages/control-plane/test/chaos-suite.test.ts)                                                                                                                                                                                                                                                                            |
| default-on telemetry or telemetry leaking deployment data                  | exact opt-in, aggregate-only D1 query, closed public schema, credential-free HTTPS sink                                               | default-off, schema stripping, adapter, and Worker wiring cases in [`packages/control-plane/test/telemetry.test.ts`](../../packages/control-plane/test/telemetry.test.ts) and [`packages/control-plane/test/worker-entry.test.ts`](../../packages/control-plane/test/worker-entry.test.ts)                                                                                                                      |
...
- The secret store encrypts records with ref-bound AES-256-GCM envelopes as defined by [ADR-005](../adr/005-secret-envelope-encryption.md). Production key provisioning, the plaintext migration procedure, provider-token rotation, and a rotation drill remain incomplete in [Issue #31](https://github.com/albert-einshutoin/TenantScript/issues/31).
