...
- The [RBAC operation matrix](rbac-matrix.md) replaces route-local role checks and documents migration from the Phase 1 `manager` claim. Persistent role bindings, scoped/revocable service tokens, and the complete escalation suite remain incomplete in [Issue #24](https://github.com/albert-einshutoin/TenantScript/issues/24).
