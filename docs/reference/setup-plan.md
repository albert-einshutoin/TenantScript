...
5. provider secret storeのDurable Object namespaceを作成する。
...
10. resourceをbindするControl Plane Workerを、selected runtime Workerとは別のownership resourceとして作成または明示adoptする。Admin mutation rate limiterのDurable Object lifecycleもこのWorker deployへ集約する。
...
lifecycle ruleを扱いません。Analytics Engineはbinding宣言であり、独立resource cleanupへ含めません。
