...
通常のreport生成contractはunknownなagent生成codeを実行しません。実行が必要な場合だけ、
[isolated runner](plugin-authoring-isolated-runner.md)が固定baseline copy、digest固定image、network deny、
resource/time limit、明示container cleanupを確立して6 judgeの閉じた結果を生成します。隔離を確立できない場合は
停止し、repository simulationを実agent成功として流用しません。LLM-as-judgeだけでpassにすることも
ありません。review済みjudge imageと実agent結果は別の公開証跡であり、runnerのrepository testだけでは
実agent性能を証明しません。
