...
- methodごとの成功status、body mode、schema IDと[成功レスポンスJSON Schema](control-plane-success-responses.md)
...
Control Planeの成功bodyは公開artifactとruntime exportの完全一致、および実handler responseの
integration testで固定します。runtime performanceはsnapshotの対象外で、benchmarkで別に守ります。
...
3. REST変更ではclient、CORS、RBAC、tenant isolation、success schema、error catalogの対応testを更新する
...
export kind変更、Control Plane REST endpoint/path/isolation/method/success contractの互換性破壊には、影響packageの
