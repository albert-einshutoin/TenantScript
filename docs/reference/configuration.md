...
| `SLACK_OAUTH_CLIENT_ID`                | Unset                                 | Slack OAuth install-start is enabled                                   | No     | Bounded Slack application client ID. Required together with scopes, redirect URI, and the OAuth state Durable Object.                                                                                                   |
| `SLACK_OAUTH_CLIENT_SECRET`            | Unset                                 | Slack OAuth callback is enabled                                        | Yes    | Slack application client secret used only by the fixed-origin server exchange. Supply only through a Worker secret binding.                                                                                             |
| `SLACK_OAUTH_FAILURE_REDIRECT_URI`     | Unset                                 | Slack OAuth callback is enabled                                        | No     | Fixed canonical HTTPS destination for every handled callback failure. Must differ from the success destination; callback input cannot override it.                                                                      |
| `SLACK_OAUTH_SCOPES`                   | Unset                                 | Slack OAuth install-start is enabled                                   | No     | Comma-separated unique least-privilege bot scopes without whitespace. Caller input cannot override this set.                                                                                                            |
| `SLACK_OAUTH_REDIRECT_URI`             | Unset                                 | Slack OAuth install-start is enabled                                   | No     | Canonical HTTPS callback URL without credentials or fragment. The same exact value must be used by code exchange.                                                                                                       |
| `SLACK_OAUTH_SUCCESS_REDIRECT_URI`     | Unset                                 | Slack OAuth callback is enabled                                        | No     | Fixed canonical HTTPS destination after a completed workspace connection. Must differ from the failure destination; callback input cannot override it.                                                                  |
