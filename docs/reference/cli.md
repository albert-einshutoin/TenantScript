...
| ------------------------ | ---------------------------------------------------------------- | --------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------- |
| **`ext init`**           | 空directoryへ安全なplugin scaffoldを生成する。                   | `--name`、`--dir`                                                           | JSON: `name`、absolute `directory`、生成`files`                                          | `0` success、`2` usage/scaffold refusal                                      | `--hook` defaultは`invoice.created`、`--type` defaultは`event`。既存fileを上書きしない。                                                |
...
| **`ext setup`**          | production self-hostのresource/cost/cleanup planを表示する。     | `--profile`、`--runtime`、`--dry-run`                                       | JSON: resource `operations`、`permissions`、`costs`、`cleanup`、`warnings`               | `0` planned、`2` usage/unsupported live apply                                | version 1は`production`と`--dry-run true`だけ。[Setup plan contract](setup-plan.md)を参照。外部mutationやcredential読込は行わない。     |
| **`ext approvals`**      | `approve`または`reject` actionでapproval decisionを送る。        | `approve` / `reject`、`--approval`、`--tenant`、`--audit-id`、`--actor`     | JSON: `approvalId`、`state`、`decidedBy`                                                 | `0` success、`1` client/request failure、`2` usage                           | `--reason`は任意。`TENANTSCRIPT_CONTROL_PLANE_URL`を設定し、認証・tenant authorityはserver側で強制する。                                |
| **`ext rollback`**       | installationを既知のversionへrollbackする。                      | `--app`、`--plugin`、`--installation`、`--to`、`--audit-id`、`--actor`      | JSON: `installationId`、`pluginVersionId`、`auditId`                                     | `0` success、`1` client/request failure、`2` usage                           | `--reason`は任意。`TENANTSCRIPT_CONTROL_PLANE_URL`が必要。[Rollback troubleshooting](../operations/rollback-troubleshooting.md)を参照。 |
