...
| **`ext audit`**          | manifestとpackage metadataの確定可能な導入前riskを検査する。               | `--manifest`、`--package`                                                                                    | JSON: version、`passed`、stable `findings`                                               | `0` errorなし、`1` error findingあり、`2` usage/read/schema failure          | V1はtest script、exact SDK pin/一致、scaffold基準超過limitsだけを判定する。unused grant、egress、test品質、license、bundle動作を保証せず、warningはhuman reviewを要求する。                                                                                                             |
