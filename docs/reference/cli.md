...
| **`ext doctor`**         | secret-freeなself-host診断reportを収集・評価する。                         | `--report`、または`--cloudflare` + `--worker`、`--database-id`、`--runtime`      | JSON: `version`、`healthy`、stable `findings`                                            | `0` healthy、`1` repair required、`2` usage/read/schema/provider failure     | live modeはread-only APIと環境変数credentialだけを使用する。[Doctor report contract](doctor-report.md)がschemaと権限境界の正本。                                                          |
