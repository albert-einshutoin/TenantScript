...
| ------------------------ | -------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **`ext init`**           | 空directoryへ安全なplugin scaffoldを生成する。                             | `--dir`と、generic scaffoldでは`--name`                                                                      | JSON: `name`、absolute `directory`、生成`files`                                          | `0` success、`2` usage/scaffold refusal                                      | `--template webhook-transformer`は名前省略可。templateは組み込みでnetwork取得せず、`--hook` / `--type`と併用不可。genericの`--hook` defaultは`invoice.created`、`--type` defaultは`event`。SDKはCLIと同じexact version。既存fileを上書きしない。                                        |
