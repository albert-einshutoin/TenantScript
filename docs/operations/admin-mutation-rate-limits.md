...
Workerから`AdminMutationRateLimitDurableObject`をexportし、Durable Object namespaceを`ADMIN_MUTATION_RATE_LIMITER_DO`としてbindingする。新規デプロイではWranglerのSQLite-backed Durable Object migrationも同時に追加する。
...
```jsonc
{
  ...
  "migrations": [
    {
      "tag": "admin-mutation-rate-limit-v1",
