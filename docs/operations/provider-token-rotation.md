...
pnpm --filter @tenantscript/control-plane exec vitest run test/provider-token-rotation-store.test.ts test/secret-store.test.ts
pnpm test:security
```

このaccountless testはfallback分類、上限、encrypted CAS、promotion/rollback/finalize、secret-free errorを
検証します。productionのtoken発行、OAuth/管理HTTP endpoint、Admin UI/CLI、provider側失効、Cloudflare
adapterのlive transactionはまだ検証しません。deploymentはこれらを追加し、実providerでcredential分類と
無停止切替を検証する必要があります。
