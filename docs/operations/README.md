...
| **Slack token rotation**    | **安全な観測 (safe observation):** app/tenant/workspace、status、generation、token ID、expiryだけを確認する。access/refresh tokenとprovider bodyは記録しない。                                                            | [Slack bot token rotation](slack-token-rotation.md)でsingle-writer、期限、crash gap、再接続手順を確認する。                                                                                                                                                    | refresh tokenを再送、古い暗号化stateへrollback、旧access tokenを先にrevokeしない。            |

Judge imageの固定digest公開とattestation検証は
[Plugin authoring judge image publication](plugin-authoring-judge-image-publication.md)を参照してください。
