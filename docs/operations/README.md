...
| **OAuth state failure**     | **安全な観測 (safe observation):** stable state error code、callback origin、session continuity、DO binding presenceを値なしで確認する。state、cookie、tenant/customer値は記録しない。                                    | [Slack OAuth install-start](slack-oauth-install-start.md)で認証・Cookie・固定認可URLを、[OAuth state store](oauth-state-store.md)でone-time consumeを、[Slack OAuth callback composition](slack-oauth-callback.md)でstate-first合成を確認する。                | stateの再利用、browser bindingのログ出力、state検証を省略したcode交換をしない。               |
