...
| **OAuth state failure**     | **安全な観測 (safe observation):** stable state error code、callback origin、session continuity、DO binding presenceを値なしで確認する。state、cookie、tenant/customer値は記録しない。                           | [OAuth state store](oauth-state-store.md)でone-time consume、browser/app/tenant/actor/redirect binding、expiry、alarm、callback未実装境界を確認する。                                                                                                          | stateの再利用、browser bindingのログ出力、state検証を省略したcode交換をしない。               |
| **Slack OAuth exchange**    | **安全な観測 (safe observation):** stable exchange code、redirect URIのallowlist一致、provider到達性をsecret-free metadataだけで確認する。authorization code、client secret、token、provider errorは記録しない。 | [Slack OAuth v2 exchange boundary](slack-oauth-exchange.md)でfixed origin、one-shot non-retry、response上限、実装済みstate storeと未実装callbackの境界を確認する。                                                                                             | timeoutや接続断後に同じcodeを再送しない。state検証なしのcallbackを公開しない。                |
