...
## Execution recording boundary

runtime callerはexecution logとusage meterを別々に組み立てず、
`createExecutionUsageRecorder`を使って記録順序とauthorityを固定できる。

```ts
const recorder = createExecutionUsageRecorder({ store, usageMeter, reportFailure });
await recorder.record({
  execution,
  metrics: { hookType: "event", cpuMs, subrequests, workflowRuns }
});
```

recorderはexecutionを先に永続化し、storeが返した`tenantId`、`pluginId`、`status`、`createdAt`だけから
usage requestを導出する。callerはruntime計測値だけを渡し、別tenantや別statusへusageを付け替えられない。
`durationMs`はwall timeなので`cpuMs`へ代用しない。execution write失敗時はmeterを呼ばず、meter失敗時は
固定された`execution_usage_recording_failed`だけを内部reporterへ渡してpersist済みexecutionを成功のまま返す。
どちらのmutationも自動retryしない。

`@tenantscript/loader/cloudflare`のproduction callerはこのrecorderへ接続し、Dynamic Worker呼び出し、
trusted capability evidence、execution永続化を1つの境界に固定する。ただし同期Loader APIの返値には
正確なCPU時間がないため、callerは推測せず`cpuMs: 0`を記録する。wall-clockの`durationMs`をCPUへ
代用してはならない。

正確なCPU利用量はWorkers Trace Events Logpushの`CPUTimeMs`をexecution IDへ非同期照合する。
Logpush受信・重複排除・後着更新と、実Analytics Engine到達は引き続きTier 2境界である。CPU照合が
未完了のrecordを課金または正確なCOGS根拠として扱わない。
...
Tier 1はD1 summaryのworkerd integration、production Workerのtenant-scoped query、Dynamic Worker caller、execution recording
boundary、Analytics Engine adapter contract、fail-open、固定field projection、tenant/plugin/date rangeを検証する。生成Wrangler version 3以降は
...
- [Cloudflare Workers Trace Events dataset](https://developers.cloudflare.com/logs/logpush/logpush-job/datasets/account/workers_trace_events/)
