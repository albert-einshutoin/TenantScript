...
これはaccountlessなcomposition contractである。production tenant runtimeからのcaller、実CPU計測、
execution ID単位のlive retry/idempotency、Analytics Engine到達は引き続き別のruntime/Tier 2境界である。
...
Tier 1はD1 summaryのworkerd integration、production Workerのtenant-scoped query、execution recording
