...
| **Admin UI**           | `pnpm --filter @tenantscript/admin-ui test`                                                                                      | このcommandはVitestとPlaywright E2Eを含む。role、認証、privileged actionは`pnpm --filter @tenantscript/admin-ui test:security`も実行し、navigation・form・dialog・tableを変えた場合は`pnpm test:admin-ui-accessibility`でaxeとkeyboard-only journeyを確認する。 | `pnpm verify`            |
