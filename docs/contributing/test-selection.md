...
| **Admin UI**           | `pnpm --filter @tenantscript/admin-ui test`                                                                                      | このcommandはVitestとPlaywright E2Eを含む。role、認証、privileged actionは`pnpm --filter @tenantscript/admin-ui test:security`も実行する。navigation・form・dialog・tableは`pnpm test:admin-ui-accessibility`、layout・responsive・主要状態は`pnpm test:admin-ui-visual`も実行する。 | `pnpm verify`            |
