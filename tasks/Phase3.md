...
- [ ] **P3-T17**(M)`ext setup`(セットアップウィザード)
  ...
  - Progress: accountless production dry-run plannerをIssue #183、最小WranglerをIssue #185、resumable run journalとownership-safe cleanupをIssue #187、fixed-origin Cloudflare API transportをIssue #189、ownership-aware D1 create/adopt/cleanup adapterをIssue #191、exact operation ID provider routerをIssue #193、pinned catalogとresumable D1 migration adapterをIssue #195で実装。live migration runner、remaining resource adapter、CLI live composition、clean-account Tier 2はIssue #34に残す
