...
- [x] **P3-T07**(S)provider 接続の汎用化
  ...
  - Evidence: `createGitHubIssueCreateProvider`、`packages/capabilities/test/capability-contracts.test.ts`、`docs/reference/provider-adapters.md`。Issue #268でSlack OAuth v2のfixed-origin code交換client、Issue #270でdigest-only・browser/app/tenant/actor/redirect-bound・one-time state store、Issue #272でstate-first・server-owned scopeのcallback service、Issue #274で認証済みinstall-start HTTP routeと`__Host-` browser binding Cookie、Issue #276でbounded callback HTTP、固定redirect、Cookie削除、Slack exchange・encrypted secret DO・sharded D1のWorker compositionを追加。refresh、Enterprise Grid authority、credential-bearing Tier 2はIssue #31に残す
...
- [ ] **P3-T16**(S)performance regression CI(**Tier 2**: 実 Cloudflare、nightly)
  ...
  - Progress: Issue #348でPhase 0 harnessを明示enableのTier 2 deploy/measurement laneへ接続し、warm/cold absolute p95、sanitized artifact、redirect/timeout/schema境界を固定。Issue #23の後続でsame/multi-tenant concurrency観測とrequest/Loader count証跡を追加。paid-plan live baselineがないため20% regression、provider billing cost、dashboardは未完了
...
- [ ] **P3-T17**(M)`ext setup`(セットアップウィザード)
  ...
  - Progress: accountless production dry-run plannerをIssue #183、最小WranglerをIssue #185、resumable run journalとownership-safe cleanupをIssue #187、fixed-origin Cloudflare API transportをIssue #189、ownership-aware D1 create/adopt/cleanup adapterをIssue #191、exact operation ID provider routerをIssue #193、pinned catalogとresumable D1 migration adapterをIssue #195、artifact/execution archiveのownership-safe R2 adapterをIssue #197、fail-closed Wrangler D1 migration runnerをIssue #199、mutation前のprovider route coverage preflightをIssue #201、strict pinned Wrangler Worker deploy processをIssue #203、Control Plane Workerのcreate/adopt ownershipとcleanup順のplan契約をIssue #205、closed initial/resume reconcile contextをIssue #207、deploy/cleanup共有の決定論的Worker target契約をIssue #209、atomic marker付きremote Worker ownership adapterをIssue #213、Durable Object lifecycleのWorker deploy集約とdeclarative `exports`移行をIssue #215、R2 adapter/renderer/cleanup共有の決定論的target契約をIssue #217、Analytics Engine binding lifecycleとD1 usage summaryのproduction compositionをIssue #264、encrypted provider secret-storeのtenant-isolated DO adapterとWorker-owned lifecycleをIssue #266、authority-safeなexecution/usage記録境界をIssue #284、Dynamic Worker production callerをIssue #288で実装。remaining resource adapter、provider-facing OAuth/key provisioning、CLI live composition、Logpush CPU reconciliation、clean-account Tier 2はIssue #34に残す
